<?php
    ...
    if(isset($_GET['id'])) {
        $movie_id = $_GET['id'];
        unset($_GET['id']);
        $movie = $client->getMovieById($movie_id);
        $title = $movie['title'];
        $year = $movie['year'];
        $plot = $movie['plot'];
        $poster = $movie['poster'];
    ...
    if(isset($_POST['title']) && isset($_POST['year']) && isset($_POST['plot']) && isset($_POST['poster-url'])) {
        ...
        try {
            $res = $client->editMovie($movie_id, $title, $year, $plot, $poster);
        }catch(Exception $ex){
            echo"<script>alert('something went wrong')</script>";
        }
    
        if($res){
            header("Location: http://localhost/movies/movie.php?id=$movie_id");
        }else {
            echo"<script>alert('something went wrong')</script>";
        }
    }
...
<head>
    ...
    <link rel="stylesheet" href="styles/editmovie.css" type="text/css"/>
...
<body>
    ...
    <div class="container-fluid m-0 pt-2 px-3" id='edit-movie-page'>
        ...
        <form 
            method='post'
            <?php
                echo "action='editmovie.php?id=$movie_id'";
            ?>
        >
